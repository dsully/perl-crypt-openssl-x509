...
This repo uses [Carton](https://metacpan.org/pod/Carton) for dependency management (`cpanfile.snapshot` is committed; deps install to gitignored `local/`). Prefix every `perl`/`make` command below with `carton exec`, e.g. `carton exec perl Makefile.PL`, `carton exec make test`.
...
`dzil build` (even run in-place without `--in`) copies the regenerated `Makefile.PL`/`LICENSE` back into the repo root via `[CopyFilesFromBuild]` — if a stale copy is already there it may prompt to overwrite.

`[GatherDir]` in `dist.ini` is not git-aware: any untracked file sitting in the working tree when `dzil build` runs gets swept into the release tarball (see `MANIFEST.SKIP`'s comments, and issue #132/release 2.1.4). Keep the tree clean, or extend `MANIFEST.SKIP`, before building.
